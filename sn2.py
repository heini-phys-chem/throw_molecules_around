...
def plot_the_stuff(c1x, c1y, c2x, c2y, XFx, XFy, XClx, XCly, XBrx, XBry):#, Hmx, Hmy):
...
#  colHm  = [ '#1f77b4' for i in range(len(Hmx))  ]
  ...
  a = np.array([XFx + XClx + XBrx])# + Hmx])
  b = np.array([XFy + XCly + XBry])# + Hmy])
  c = np.array([colF + colCl + colBr])# + colHm])
  ...
  #plt.legend(handles=[Hm, F, Cl, Br, C ])
  ...
  fig.savefig('sn2_X.png')
  fig.savefig('sn2_X.pdf')
  #plt.show()
...
#      Hmy.append(Y[2])
  ...
  plot_the_stuff(c1x, c1y, c2x, c2y, XFx, XFy, XClx, XCly, XBrx, XBry)#, Hmx, Hmy)
